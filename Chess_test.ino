...
const uint16_t m_otvet[456] PROGMEM = {
      ...
      };             // 1011 010 011 101 110 
                     // 0100 101 111 101 110
                     // 1010 100 100 010 011
             
uint8_t u_pozic = 97, not_x = 2, not_y = 1, rb = 0, cod_col = 0, cod_cnt = 0, u_menu = 0 ;
uint8_t col_fig = 6, col_fon = 1, col_fw = 2, col_fb = 4 ;
int smesh_d = 150, uk_mas = 0, uk_i = 0, uk_j = 0, uk_cnt = 0, m_poz_u =0 ;  
long Lbat1 = 0, Lbat2 = 0;
uint32_t Wbat1 = 0, Wbat2 = 0;
M5EPD_Canvas canvas(&M5.EPD);
M5EPD_Canvas canvas1(&M5.EPD);
M5EPD_Canvas canvas2(&M5.EPD);

void Sprite_Figur(uint8_t kod_fig){
 if ((kod_fig&8)==8) {
  col_fig = 0;} else {
  col_fig = 7;}
 if((not_x + not_y)&1==1){
...
void Out_Figur(uint8_t kod_fig) {
 ...
 for (uk_j = 0; uk_j < 58; uk_j++){
  ...
  while ( uk_i < 60){
   ...
   cod_col = (rb&0xc0) >> 6; cod_cnt = (rb & 0x3f);
   for (uk_cnt = 0; uk_cnt < cod_cnt; uk_cnt++){
    if (cod_col == 0) { canvas.drawPixel(not_x*67+uk_i+6, smesh_d+9+not_y*67+uk_j, col_fon); uk_i++; };
    if (cod_col == 1) { canvas.drawPixel(not_x*67+uk_i+6, smesh_d+9+not_y*67+uk_j, 15 ); uk_i++; };
    if (cod_col == 2) { canvas.drawPixel(not_x*67+uk_i+6, smesh_d+9+not_y*67+uk_j, col_fig); uk_i++; }
   }
  }
 }
}
 ...
 if( M5.BtnP.wasPressed()) {
  if (u_menu == 0) {
   ...
   for (int32_t i_otv = m_otv_u[u_pozic]; i_otv < m_otv_u[u_pozic+1]; i_otv++){
    ...
    //for (int32_t i_blink = 0; i_blink < 2; i_blink++){
     Sprite_Figur(fig_r);   
